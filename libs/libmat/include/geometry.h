...
class Geometry
{
	public:
		...
		double ratio() const
		{ return (double) std::max(w, h) / std::min(w, h); }
...
class Vector 
{
	public:
		...
		bool is_zero(T threshold = 1e-6) const
		{ return fabs(x) < threshold && fabs(y) < threshold && fabs(z) < threshold; }

		bool is_positive(T threshold = 1e-6) const
...
class Vector2D
{
	public:
		...
		{ return fabs(x - v.x) < 1e-6 && fabs(y - v.y) < 1e-6; }
		...
		void rot90_clock(T w, T h)
		{
			T neww = h, newh = w;
			//printf("(w,h):(%g,%g)\n", w,h);
			//printf("rot90_clock:before:(x:y):(%g,%g)\n", x,y);
			transpose(neww,newh);
			flip_x(neww,newh);
			//printf("rot90_clock:after:(x:y):(%g,%g)\n", x,y);
			//getchar();
		}
		void rot90_counter_clock(T w, T h)
		{
			T neww = h, newh = w;
			//printf("(w,h):(%g,%g)\n", w,h);
			//printf("rot90_counter_clock:before:(x:y):(%g,%g)\n", x,y);
			transpose(neww,newh);
			flip_y(neww,newh);
			//printf("rot90_counter_clock:after:(x:y):(%g,%g)\n", x,y);
			//getchar();
		}
		void rot180(T w, T h)
		{
			//printf("(w,h):(%g,%g)\n", w,h);
			//printf("rot180:before:(x:y):(%g,%g)\n", x,y);
			flip_x(w,h);
			flip_y(w,h);
			//printf("rot180:after:(x:y):(%g,%g)\n", x,y);
			//getchar();
		}
};
