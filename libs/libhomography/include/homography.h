...
class Homography 
{
	public:
		...
		void operator += (const Homography& r) 
		{ 
			for(int i=0;i<9;++i)
				data[i] += r.data[i]; 
		}
		...
		static bool health(const double* mat) 
        {
            ...
            Vec	x1{ mat[1]+mat[2],mat[4]+mat[5],mat[7]+mat[8] };	// trans(0,1)
                         
			if (x1.y <= x0.y)
				return false;
            
			Vec x2{mat[0]+mat[1]+mat[2], mat[3]+mat[4]+mat[5], mat[6]+mat[7]+mat[8]};	// trans(1, 1);
