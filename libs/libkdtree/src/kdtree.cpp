...
KDTree::KDTree(const std::vector<const Point*> & points) 
{
...
KDTree::DimStats KDTree::build_dim_stat(const std::vector<const Point*>& pts) 
{
	DimStats ret;
...
KDTree::Node* KDTree::build(vector<PointInBuild> & points,DimStats& stat, int depth) 
{
...
KDTree::NNResult KDTree::nn_in_node(const Point& p, Node* n, float thres) const 
{
	if (n->leaf()) 
	{
		...
		for (auto& lp: n->pts) 
		{
			float v = dist_euclidean_sqr(lp.second->data(), p.data(), D, min_d);
			if (min_d > v)
			{
				min_d = v;
				min_i = lp.first;
...
KDTree::TwoNNResult KDTree::two_nn_in_node(const Point& p, Node* n, float thres) const 
{
	if (n->leaf()) 
	{
		...
		for (auto& lp: n->pts) 
		{		// leaf point
			float v = dist_euclidean_sqr(lp.second->data(), p.data(), D, min_d2);
			if (v < min_d)
			{
	...
	else 
	{
		...
		if (ret.sqrdist < dist) {		// second largest might be in another part
			...
			if (ret.sqrdist2>ret2.sqrdist) 
				ret.sqrdist2 = ret2.sqrdist;
		...
		if (ret2.sqrdist < ret.sqrdist) 
		{
			...
			if (ret.sqrdist2 > ret2.sqrdist2) 
				ret.sqrdist2=ret2.sqrdist2;
		}
		else
		{
			if (ret.sqrdist2 > ret2.sqrdist) 
