...
const int DESC_HIST_SCALE_FACTOR =3;
		...
		float weight, float hist[][DESC_HIST_BIN_NUM]) 
{
	...
	xbinf = floor(xbin),
	hbinf = floor(hbin);
	float ybind = ybin - ybinf,xbind = xbin - xbinf,hbind = hbin - hbinf;
...
Descriptor SIFT::calc_descriptor(const SSPoint& p) const {
	...
	exp_denom = 2 *(DESC_HIST_WIDTH*DESC_HIST_WIDTH);
	...
	for (int xx = -radius; xx <= radius; xx ++) {
		...
		for (int yy = -radius; yy <= radius; yy ++) {
			...
			if ((xx*xx + yy*yy) > (radius*radius)) continue;		// to be circle
			...
			float weight = expf(-(x_rot*x_rot + y_rot*y_rot) / exp_denom);
