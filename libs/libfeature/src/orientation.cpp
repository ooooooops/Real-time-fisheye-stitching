...
const float ORI_RADIUS = 4.5;// # this radius might be large?	
const int ORI_HIST_SMOOTH_COUNT =2;
...
std::vector<float> OrientationAssign::calc_dir(
	...
	float exp_denom = 2 * (gauss_weight_sigma*gauss_weight_sigma);
	...
	for (int xx = -rad; xx < rad; xx ++) {
		...
		for (int yy = -rad; yy < rad; yy ++) {
		  ...
		  // use a circular gaussian window
			...
			float weight = expf(-(xx*xx + yy*yy) / exp_denom);
	...
	for (int K = ORI_HIST_SMOOTH_COUNT; K --;)
		for(int i=0;i<ORI_HIST_BIN_NUM;++i) {
	...
	for(int i=0;i<ORI_HIST_BIN_NUM;++i) 
	{
		...
		if (hist[i] > thres && hist[i] > max(prev, next)) {
			...
			double newbin = (float)i - 0.5 + (hist[i] - prev) / (prev + next - 2 * hist[i]);
