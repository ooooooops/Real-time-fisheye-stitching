...
//CONTRAST_THRES 4e-2
const float CONTRAST_THRES = 3e-2;//Lowe: 3e-2. smaller value gives more feature	

const float JUDGE_EXTREMA_DIFF_THRES = 2e-3;//smaller value gives more feature
//#!! making it small could result in low-quality keypoint
const float EDGE_RATIO = 10;//lowe: 10. larger value gives more feature

const float PRE_COLOR_THRES = 5e-2;	
const float CALC_OFFSET_DEPTH = 4;
const float OFFSET_THRES = 0.5;//#0.3 is still good, this settings has big impact
...
bool ExtremaDetector::calc_kp_offset(SSPoint* sp) const {
	...
	for(;niter < CALC_OFFSET_DEPTH; ++niter) {
		if (!between(nowx, 1, w - 1) || !between(nowy, 1, h - 1) || !between(nows, 1, nscale - 2))
			return false;
...
bool ExtremaDetector::is_edge_response(Coor coor, const Mat32f& img) const {
	...
	float tr2 = (dxx + dyy)*(dxx + dyy);

	// Calculate principal curvature by hessian
	if (tr2 / det <((EDGE_RATIO + 1)*(EDGE_RATIO + 1))/EDGE_RATIO) 
		return false;
