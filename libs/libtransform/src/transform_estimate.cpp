...
const int RANSAC_ITERATIONS = 1500;// # lowe: 500
const double RANSAC_INLIER_THRES = 3.5;// # inlier threshold corresponding to 800-resolution images
const float INLIER_MINIMUM_RATIO = 0.1;//	# number of inlier divided by all matches in the overlapping region
//# ----
...
bool TransformEstimation::get_transform(MatchInfo* info) 
{
	...
	if (nr_match < nr_match_used) return false;
...
vector<int> TransformEstimation::get_inliers(const Homography& trans) const {
	float INLIER_DIST = (ransac_inlier_thres*ransac_inlier_thres);
...
bool TransformEstimation::fill_inliers_to_matchinfo(
	...
	printf("r1mr1p: %lf,%lf, r2mr2p: %lf,%lf\n", r1m, r1p, r2m, r2p);
